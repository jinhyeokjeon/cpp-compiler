...
or 연산자는 대입 연산자보다 우선순위가 한 단계 높으므로 parseAssignment() 함수로부터 호출된다. 시작은 마찬가지로 왼쪽 피연산자 식을 분석하기 위해 or 연산자보다 우선순위가 한 단계 더 높은 and 연산자를 분석하는 것이다.
```cpp
...
왼쪽 피연산자 식을 분석한 후에는 연산자가 나올 차례다. 연속된 or 연산자들을 분석하기 위해 현재 토큰이 LogicalOr일때까지 반복한다.

대입 연산자는 결합방향이 우에서 좌이므로(오른쪽 식을 먼저 평가하고 왼쪽 식을 평가한다) 재귀 호출을 하며 분석했지만, or 연산자는 좌에서 우 이므로(왼쪽 식을 먼저 평가하고 오른쪽 식을 평가한다) 반복하여 분석한다.
```cpp
while (skipCurrentIf(Kind::LogicalOr)) {
}
```

현재 토큰이 or 연산자라면 or 연산자 노드를 생성한다. 이어서 왼쪽 피연산자 식을 앞서 분석해 두었던 노드로 설정하고 오른쪽 피연산자 식은 왼쪽 식을 분석했을 때와 마찬가지로 우선 순위가 한 단계 더 높은 and 연산자를 분석하는 함수를 호출해 설정한다. 그리고 분석 결과가 새로 생성한 or 연산자 노드가 되도록 result 변수를 새로 생성한 or 연산자 노드로 설정한다.

현재 토큰이 or 연산자가 아니라면 result 변수를 반환하며 분석을 종료한다.
```cpp
Expression* temp = new Or();
temp->lhs = result;
temp->rhs = parseAnd();
result = temp;
```
...
- 구문 분석은 컴파일 과정의 두 번째 단계이다.
- 구문 분석은 소스 코드의 구조를 분석하는 것이다.
- 소스 코드의 구조를 표현하는 트리를 구문 트리라 한다.
- 소스 코드의 구조는 문과 식으로 구성된다.
- 문은 문과 식을 포함하고 식은 식을 포함한다.
- 연산자에는 우선순위가 있다.
- 연산자의 우선 순위가 높을수록 구문 트리의 잎에 가깝게 위치한다.
- 구문 트리는 자식이 없는 잎 노드부터 평가한다.
- 연산자에는 결합 방향이 있으며, 결합 방향에는 왼쪽과 오른쪽이 있다.
- 결합 방향이 왼쪽인 연산자는 구문 트리를 왼쪽으로 키우고 왼쪽부터 계산한다.
- 결합 방향이 오른쪽인 연산자는 구문 트리를 오른쪽으로 키우고 오른쪽부터 계산한다.
- 구문 분석기는 소스 코드의 구조를 분석하는 구문 분석 프로그램이다.
- 구문 분석기의 입력은 토큰 리스트고, 출력은 구문 트리다.